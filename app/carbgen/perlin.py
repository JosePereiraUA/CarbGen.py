...
if __name__ == 'pymol':
    ...
    def treta(selection, obj, threshold=0.5, octave=1):
        threshold = float(threshold)
        octave = int(octave)
        perlin = PerlinNoiseFactory(2, octaves=octave)
        cmd.create(obj, selection, 1, 1)
        model = cmd.get_model(obj)
        for atom in model.atom:
            n = perlin(atom.coord[0], atom.coord[1])
            if n>threshold:
                #atom.symbol = 'S'
                cmd.alter('%s and id %d' % (obj, atom.id), 'elem="S"')
                # atom.elem = 'S'
                # print dir(atom)
        #cmd.rebuild()
        cmd.color('red', 'e. S')

    cmd.extend('treta', treta)